 ...
 *
 */
public class RatingResultTest {

	RatingResult ratResult = new RatingResult(IDGenerator.next(), 3);

	@Test
	public void testMakeProfileDescription() {
		ratResult = new RatingResult(IDGenerator.next(), 3);
	}

	@Test
	public void testgetIdAttribute() throws NoSuchFieldException,
			SecurityException {
		Field field = ratResult.getClass().getDeclaredField("id");
		field.setAccessible(true);
		assertEquals(ratResult.getIdAttribute(), new Attribute(field.getName(),
	...
	public void testToString() {
		Field[] fields = ratResult.getClass().getDeclaredFields();
		String strFields = ratResult.toString();
		for (Field field : fields) {
			field.setAccessible(true);
			// Ignore private static fields && private static final fields
			if (field.getModifiers() != 10 && field.getModifiers() != 26) {
