...
import main.foundation.evaluation.LMSEvaluationReport;
...
/**
 * Class that visualizes the result of an evaluation in a Swing frame. It
 * generates a chart with the evaluation result an other information returned by
 * the evaluator.
 * 
 ...
 * 
 */
public class LMSEvaluationResultGUI extends EvaluationResultGUI {

	private static Chart2D chart;
	private static JFrame dialog;
	private static LMSEvaluationReport evalReport;

	public static void show(LMSEvaluationReport er, String title,
			boolean exitOnClose) {
		evalReport = er;

		dialog = new JFrame();
		dialog.setTitle("LMSRecommender Evaluation");
		...
		// data.add(Box.createGlue());
		// data.add( new JLabel("Average: "+
		// String.format("%6f",er.getEvaluationAverage())));
		dialog.getContentPane().add(data, BorderLayout.NORTH);

		chart = getChart(title, er);
		chart.setMaximumSize(new Dimension(860, 320));
		...
		dialog.getContentPane().add(chart, BorderLayout.WEST);
		...
		exportData.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					FileDialog fd = new FileDialog(dialog, "Save as CSV",
							FileDialog.SAVE);
		...
		exportChart.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					FileDialog fd = new FileDialog(dialog, "Save as JPG",
							FileDialog.SAVE);
		...
		dialog.setPreferredSize(screenSize/* new Dimension(860, 320) */);
		dialog.pack();
		dialog.doLayout();
	...
	// FIXME BUG #1 Error of showing the chart in the dialog of
	// main.userinterface.LMSEvaluationResultGUI.
	// Work-around: Minimize and maximize the size of the dialog to render the
	// chart again and it should show it correctly.
	private static Chart2D getChart(String title, LMSEvaluationReport er) {
		...
		// String[] labelsAxisLabels = new String[10];
		// int numCycles = er.getNumberOfCycles();
		// int numLabels = numCycles / labelsAxisLabels.length;
		// for (int i = 0; i < labelsAxisLabels.length - 1; i++) {
		// labelsAxisLabels[i] = String.valueOf(numLabels - 1 * (i + 1));
		// }
		...
		// System.out.println("lines: " + lines + ", " + "lineSize: " + lineSize
		// + ", " + "numCats: " + dataset.getNumCats() + ", "
		// + "numTicks: "
	...
	static void saveEvaluationToCSV(LMSEvaluationReport er, File file)
			throws IOException {
	...
	static void saveComponentToJPG(Component component, File file)
			throws IOException {
