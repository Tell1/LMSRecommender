 ...
 * 
 */
public class LMSObtainQueryWithFormMethod extends ObtainQueryWithFormMethod {
	private static Hashtable<Attribute, ParameterEditor> editors;
	private static JDialog dialog;

	/**
	 * Obtains a query without showing initila values. It shows every attribute
	 * and uses the attribute name as the label.
	 * 
	 * @param query
	 *            to obtain.
	 */
	public static void obtainQueryWithoutInitialValues(CBRQuery query,
			Collection<Attribute> hiddenAttributes,
	 ...
	 * 
	 * @param query
	 *            with the initial values and where the user new values are
	 *            stored
	 */
	 ...
	 * 
	 * @param query
	 *            to fill with the obtained values and containing the initial
	 *            values
	 * @param useQueryvalues
	 *            indicates if the query values are shown by default
	 * @param hiddenAttributes
	 *            is a list of not shown attributes
	 * @param labels
	 *            for each attribute. If there is no label for an attribute, the
	 *            attribute name is used.
	 */
	static void obtainQuery(CBRQuery query, boolean useQueryvalues,
			Collection<Attribute> hiddenAttributes,
			Map<Attribute, String> labels) {
		dialog = new JDialog();
		...
		if (useQueryvalues) {
			for (Attribute a : editors.keySet()) {
				...
				editor.setEditorValue(jcolibri.util.AttributeUtils.findValue(a,
						query));
			}
		}
	...
	/**
	 * Adds Parameter editor to the panel for each attribute in the
	 * CaseComponent
	 * 
	 * @param cc
	 *            CaseComponent with the attributes
	 * @param panel
	 *            to add the ParameterEditor
	 * @param hiddenAttributes
	 *            attributes not shown
	 * @param labels
	 *            for the attributes
	 */
	private static void addAttributes(CaseComponent cc, JPanel panel,
			Collection<Attribute> hiddenAttributes,
			Map<Attribute, String> labels) {
		...
		try {
			...
			Attribute[] ats = jcolibri.util.AttributeUtils.getAttributes(cc
					.getClass());
			...
			for (Attribute a : ats) {
				if (a.getType().equals(CaseComponent.class)) {
					...
					ParameterEditor pe = ParameterEditorFactory.getEditor(a
							.getType());
			...
			for (Attribute comp : compounds) {
				...
				subpanel.setBorder(javax.swing.BorderFactory
						.createTitledBorder(comp.getName()));
				addAttributes((CaseComponent) comp.getValue(cc), subpanel,
						hiddenAttributes, labels);
			...
			org.apache.commons.logging.LogFactory.getLog(AttributeUtils.class)
					.error(e);
		}
	}
